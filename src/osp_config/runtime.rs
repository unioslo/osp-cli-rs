...
impl RuntimeDefaults {
    ...
    pub fn get_string(&self, key: &str) -> Option<&str> {
        self.layer
            .entries()
            .iter()
            .find(|entry| entry.key == key && entry.scope == crate::osp_config::Scope::global())
            .and_then(|entry| match entry.value.reveal() {
                crate::osp_config::ConfigValue::String(value) => Some(value.as_str()),
                _ => None,
            })
    }
...
mod tests {
    ...
    use crate::osp_config::{ConfigLayer, ConfigValue, Scope};
