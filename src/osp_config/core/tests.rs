...
fn bootstrap_key_registry_rejects_profile_scopes() {
    ...
    match err {
        crate::osp_config::ConfigError::InvalidBootstrapScope {
            key,
            profile,
            terminal,
        } => {
...
fn parse_input_value_adapts_scalars_lists_and_enums() {
    ...
    assert!(matches!(
        enum_err,
        crate::osp_config::ConfigError::InvalidEnumValue {
            key,
            value,
            allowed
    ...
    assert!(matches!(
        type_err,
        crate::osp_config::ConfigError::InvalidValueType {
            key,
...
fn config_values_handle_secrets_display_and_interpolation() {
    ...
    assert!(matches!(
        err,
        crate::osp_config::ConfigError::NonScalarPlaceholder { key, placeholder }
...
fn bootstrap_value_and_alias_helpers_match_runtime_rules() {
    ...
    assert!(matches!(
        err,
        crate::osp_config::ConfigError::InvalidBootstrapValue { key, .. } if key == "profile.default"
    ));
}
...
fn config_value_from_toml_rejects_tables_and_preserves_datetime_strings() {
    ...
    assert!(matches!(
        err,
        crate::osp_config::ConfigError::UnsupportedTomlValue { path, kind }
...
fn normalize_scope_lowercases_profile_and_terminal_names() {
    let normalized = crate::osp_config::normalize_scope(Scope {
...
fn validate_and_adapt_rejects_unknown_and_missing_required_keys() {
    ...
    assert!(matches!(
        err,
        crate::osp_config::ConfigError::UnknownConfigKeys { keys } if keys == vec!["mystery.key".to_string()]
    ));
    ...
    assert!(matches!(
        err,
        crate::osp_config::ConfigError::MissingRequiredKey { key } if key == "profile.active"
    ));
}
...
fn parse_input_value_covers_float_unknown_and_allowed_scope_paths_unit() {
    ...
    assert!(matches!(
        schema
            ...
            .expect_err("unknown keys should fail"),
        crate::osp_config::ConfigError::UnknownConfigKeys { keys } if keys == vec!["not.real".to_string()]
    ));
...
fn config_layer_from_toml_str_reports_invalid_sections_and_unknown_roots_unit() {
    assert!(matches!(
        ...
        crate::osp_config::ConfigError::InvalidSection { section, expected }
    ...
    assert!(matches!(
        ...
        crate::osp_config::ConfigError::UnknownTopLevelSection(section) if section == "mystery"
    ));
}
...
fn config_layer_helpers_cover_insert_remove_and_profile_terminal_scope_unit() {
    ...
    assert_eq!(crate::osp_config::normalize_identifier(" RePl "), "repl");
}
...
fn schema_adaptation_helpers_cover_scalar_secret_list_and_env_paths_unit() {
    ...
    assert!(matches!(
        ...
        crate::osp_config::ConfigError::InvalidValueType { key, .. } if key == "demo.bool"
    ));
    assert_eq!(
        adapt_value_for_schema(
            "demo.int",
    ...
    assert!(matches!(
        ...
        crate::osp_config::ConfigError::InvalidValueType { key, .. } if key == "demo.float"
    ));
    assert_eq!(
        adapt_value_for_schema(
            "demo.list",
    ...
    assert!(matches!(
        ...
        crate::osp_config::ConfigError::InvalidValueType { key, .. } if key == "demo.list"
    ));
    assert_eq!(
        adapt_value_for_schema(
            "demo.secret_list",
    ...
    assert!(matches!(
        ...
        crate::osp_config::ConfigError::InvalidEnvOverride { reason, .. } if reason.contains("missing OSP__ prefix")
    ));
    assert!(matches!(
        parse_env_key("OSP__TERM").err().expect("term requires name"),
        crate::osp_config::ConfigError::InvalidEnvOverride { reason, .. } if reason.contains("TERM requires a terminal name")
    ));
    assert!(matches!(
        parse_env_key("OSP__PROFILE").err().expect("profile requires name"),
        crate::osp_config::ConfigError::InvalidEnvOverride { reason, .. } if reason.contains("PROFILE requires a profile name")
    ));
    assert!(matches!(
        parse_env_key("OSP__TERM__a__TERM__b__UI__FORMAT")
            .err()
            .expect("duplicate TERM should fail"),
        crate::osp_config::ConfigError::InvalidEnvOverride { reason, .. } if reason.contains("TERM scope specified more than once")
    ));
    assert!(matches!(
        parse_env_key("OSP__PROFILE__a__PROFILE__b__UI__FORMAT")
            .err()
            .expect("duplicate PROFILE should fail"),
        crate::osp_config::ConfigError::InvalidEnvOverride { reason, .. } if reason.contains("PROFILE scope specified more than once")
    ));
    assert!(matches!(
        parse_env_key("OSP____").err().expect("missing key path should fail"),
        crate::osp_config::ConfigError::InvalidEnvOverride { reason, .. } if reason.contains("missing key path")
    ));
    assert!(matches!(
        parse_env_key("OSP__TERM__repl")
            .err()
            .expect("missing final key should fail"),
        crate::osp_config::ConfigError::InvalidEnvOverride { reason, .. } if reason.contains("missing final config key")
    ));
}
