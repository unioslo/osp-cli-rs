use crate::osp_completion::model::{
    CommandLine, CursorState, FlagOccurrence, ParsedLine, QuoteStyle,
};
...
impl ParseState {
    ...
    fn collect_positional_tail<'a>(
        ...
        while let Some(next) = iter.next() {
            ...
            self.tail
                .push(crate::osp_completion::model::TailItem::Positional(
                    next.clone(),
                ));
        }
    }
    ...
    fn parse_flag_tail<'a>(
        ...
        loop {
            ...
            if let Some((flag, value)) = split_inline_flag_value(&current) {
                ...
                self.tail.push(crate::osp_completion::model::TailItem::Flag(
                    FlagOccurrence {
                        name: flag.clone(),
