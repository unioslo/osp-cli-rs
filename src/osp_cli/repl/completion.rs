use crate::osp_cli::app::CMD_CONFIG;
use crate::osp_cli::invocation::scan_command_tokens;
use crate::osp_cli::pipeline::{is_cli_help_stage, validate_cli_dsl_stages};
use crate::osp_completion::{
    ArgNode, CommandLineParser, CompletionNode, CompletionTree, CompletionTreeBuilder,
    ContextScope, FlagNode, SuggestionEntry,
};
use crate::osp_dsl::parse::pipeline::parse_stage;
use crate::osp_dsl::{VerbStreaming, registered_verbs, render_streaming_badge, verb_info};
use crate::osp_repl::default_pipe_verbs;
...
struct AliasCompletionCommand {
    command: crate::osp_completion::CommandLine,
...
fn alias_completion_command(template: &str) -> Option<AliasCompletionCommand> {
    ...
    let parsed = crate::osp_dsl::parse_pipeline(&sanitized).ok()?;
...
mod tests {
    ...
    use crate::osp_cli::repl::surface::ReplAliasEntry;
    use crate::osp_cli::state::{
        AppState, AppStateInit, LaunchContext, ReplScopeStack, RuntimeContext, TerminalKind,
    };
    use crate::osp_completion::{CompletionNode, CompletionTree, ContextScope, FlagNode};
