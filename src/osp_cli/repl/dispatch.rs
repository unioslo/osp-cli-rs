...
For more information, try '--help'.\n";
    ...
    fn root_help_rendering_and_shell_prefix_helpers_cover_root_paths_unit() {
        let mut state = make_state_with_plugins(
            crate::osp_cli::plugin_manager::PluginManager::new(Vec::new()),
        );
...
"#,
        ...
        let mut state =
            make_state_with_plugins(crate::osp_cli::plugin_manager::PluginManager::new(vec![
                plugins_dir.clone(),
            ]));
        let invocation = super::base_repl_invocation(&state.runtime);
