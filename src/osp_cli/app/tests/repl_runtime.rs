...
"#,
    ...
    match first {
        crate::osp_repl::ReplLineResult::Continue(text) => assert!(text.contains("ok")),
...
"#,
    ...
    match first {
        crate::osp_repl::ReplLineResult::Continue(text) => assert!(text.contains('1')),
        other => panic!("unexpected repl result: {other:?}"),
    }
    ...
    match second {
        crate::osp_repl::ReplLineResult::Continue(text) => assert!(text.contains("cached")),
        other => panic!("unexpected repl result: {other:?}"),
    }
...
fn plugin_pipeline_rendering_matches_between_cli_and_repl_unit() {
    ...
    match repl_rendered {
        crate::osp_repl::ReplLineResult::Continue(text) => {
...
fn repl_reload_intent_matches_command_scope_unit() {
    ...
    state.runtime.themes =
        crate::osp_cli::theme_loader::load_theme_catalog(state.runtime.config.resolved());
    let history = make_test_history(&mut state);
    ...
    assert!(matches!(
        theme_result,
        crate::osp_repl::ReplLineResult::Restart {
            reload: crate::osp_repl::ReplReloadKind::WithIntro,
            ..
        }
    ));
    ...
    assert!(matches!(
        format_result,
        crate::osp_repl::ReplLineResult::Restart {
            reload: crate::osp_repl::ReplReloadKind::Default,
            ..
        }
    ));
    ...
    assert!(matches!(
        ...
        crate::osp_repl::ReplLineResult::Continue(_)
    ));
}
...
fn repl_config_prompt_color_change_rebuilds_deterministically_unit() {
    ...
    let default_prompt = crate::osp_cli::repl::presentation::build_repl_prompt(repl_view(
        &state.runtime,
        &state.session,
    ))
    .left;
    ...
    state = super::super::rebuild_repl_state(&state).expect("rebuild should succeed");
    let white_prompt = crate::osp_cli::repl::presentation::build_repl_prompt(repl_view(
        &state.runtime,
        &state.session,
    ))
    .left;
    ...
    state = super::super::rebuild_repl_state(&state).expect("rebuild should succeed");
    let restored_prompt = crate::osp_cli::repl::presentation::build_repl_prompt(repl_view(
        &state.runtime,
        &state.session,
    ))
    .left;
...
fn repl_exit_is_host_owned_at_root_but_leaves_shell_in_scope_unit() {
    ...
    assert_eq!(root_exit, crate::osp_repl::ReplLineResult::Exit(0));
...
fn repl_failure_is_cached_for_doctor_last_unit() {
    ...
    let rendered = doctor_cmd::run_doctor_command(
        doctor_cmd::DoctorCommandContext {
            config: crate::osp_cli::cli::commands::config::ConfigReadContext {
            ...
            plugins: crate::osp_cli::cli::commands::plugins::PluginsCommandContext {
                config: state.runtime.config.resolved(),
        ...
        crate::osp_cli::cli::DoctorArgs {
            command: Some(crate::osp_cli::cli::DoctorCommands::Last),
        },
    )
...
"#,
    ...
    match expanded {
        crate::osp_repl::ReplLineResult::ReplaceInput(text) => {
...
fn repl_bang_contains_search_respects_shell_scope_unit() {
    ...
    match scoped_hit {
        crate::osp_repl::ReplLineResult::ReplaceInput(text) => assert_eq!(text, "status"),
        other => panic!("unexpected repl result: {other:?}"),
    }
...
fn repl_help_alias_keeps_colored_help_chrome_and_subcommands_unit() {
    ...
    state.runtime.ui.render_settings.style_overrides = crate::osp_ui::style::StyleOverrides {
        panel_title: Some("green".to_string()),
        key: Some("red".to_string()),
        value: Some("blue".to_string()),
        ..crate::osp_ui::style::StyleOverrides::default()
    };
    let history = make_test_history(&mut state);
...
fn repl_flag_prefixed_help_records_prompt_timing_badge_unit() {
    ...
    let prompt_right = crate::osp_cli::repl::render_repl_prompt_right_for_test(
