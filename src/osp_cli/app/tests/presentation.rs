...
fn repl_simple_prompt_shows_shell_scope_indicator_unit() {
    ...
    let prompt = crate::osp_cli::repl::presentation::build_repl_prompt(repl_view(
        &state.runtime,
        &state.session,
    ))
    .left;
...
fn help_render_overrides_parse_long_flags_unit() {
    ...
    assert_eq!(
        parsed.mode,
        Some(crate::osp_core::output::RenderMode::Plain)
    );
    assert_eq!(
        parsed.color,
        Some(crate::osp_core::output::ColorMode::Always)
    );
    assert_eq!(
        parsed.unicode,
        Some(crate::osp_core::output::UnicodeMode::Never)
    );
