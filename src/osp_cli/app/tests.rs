...
fn render_prompt_snapshot(entries: &[(&str, &str)]) -> String {
    ...
    crate::osp_cli::repl::presentation::build_repl_prompt(repl_view(&state.runtime, &state.session))
        .left
}
...
fn render_table_snapshot(entries: &[(&str, &str)]) -> String {
    ...
    render_output(
        &crate::osp_cli::rows::output::rows_to_output_result(rows),
        &settings,
    )
}
...
fn repl_dsl_capability_is_declared_per_command_unit() {
    ...
    let history_prune = Commands::History(crate::osp_cli::cli::HistoryArgs {
        command: crate::osp_cli::cli::HistoryCommands::Prune(
            crate::osp_cli::cli::HistoryPruneArgs { keep: 5 },
        ),
    });
