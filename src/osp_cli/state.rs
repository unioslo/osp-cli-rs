...
use crate::osp_repl::HistoryShellContext;
...
use crate::osp_cli::plugin_config::{PluginConfigEntry, PluginConfigEnv, PluginConfigEnvCache};
...
fn configured_usize(config: &ResolvedConfig, key: &str, fallback: usize) -> usize {
    match config.get(key).map(crate::osp_config::ConfigValue::reveal) {
        Some(crate::osp_config::ConfigValue::Integer(value)) if *value > 0 => *value as usize,
        Some(crate::osp_config::ConfigValue::String(raw)) => raw
            .trim()
            .parse::<usize>()
            .ok()
