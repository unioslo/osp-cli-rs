use crate::osp_completion::{ArgNode, CommandSpec, FlagNode, SuggestionEntry};
use crate::osp_config::{ConfigSchema, SchemaValueType};
...
pub(crate) fn collect_alias_entries(config: &crate::osp_config::ResolvedConfig) -> Vec<ReplAliasEntry> {
    let mut out = Vec::new();
...
pub(crate) fn config_set_key_specs() -> Vec<crate::osp_completion::ConfigKeySpec> {
    let schema = ConfigSchema::default();
    schema
        .entries()
        .map(|(key, entry)| {
            ...
            crate::osp_completion::ConfigKeySpec::new(key).value_suggestions(value_suggestions)
        })
        .collect()
}
