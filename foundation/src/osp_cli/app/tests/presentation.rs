...
fn repl_prompt_right_shows_ascii_incognito_marker_unit() {
    ...
    let timing = crate::osp_cli::state::DebugTimingState::default();
...
fn repl_prompt_right_includes_timing_breakdown_at_debug_three_unit() {
    ...
    let resolved = settings.resolve_render_settings();
    let timing = crate::osp_cli::state::DebugTimingState::default();
    timing.set(crate::osp_cli::state::DebugTimingBadge {
        level: 3,
        summary: crate::osp_cli::app::TimingSummary {
...
fn austere_repl_intro_is_minimal_single_line_unit() {
    ...
    let rendered =
        crate::osp_cli::repl::presentation::render_repl_intro(repl_view(&state.runtime, &state.session));
...
fn help_render_overrides_parse_long_flags_unit() {
    ...
    assert_eq!(parsed.mode, Some(crate::osp_core::output::RenderMode::Plain));
    assert_eq!(parsed.color, Some(crate::osp_core::output::ColorMode::Always));
    assert_eq!(parsed.unicode, Some(crate::osp_core::output::UnicodeMode::Never));
...
fn help_chrome_uses_unicode_dividers_when_enabled_unit() {
    ...
    let rendered = repl_help::render_help_with_chrome(
        "Usage: osp [OPTIONS]\n\nCommands:\n  help\n\nOptions:\n  -h, --help\n",
        &resolved,
        crate::osp_cli::ui_presentation::HelpLayout::Full,
    );
    assert!(rendered.contains("Usage"));
