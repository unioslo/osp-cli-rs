...
use crate::osp_cli::plugin_manager::{
...
use clap::Parser;
use crate::osp_config::{ConfigLayer, ConfigResolver, ConfigValue, ResolveOptions, RuntimeLoadOptions};
use crate::osp_core::output::{ColorMode, OutputFormat, RenderMode, UnicodeMode};
use crate::osp_core::plugin::{
    ResponseErrorV1, ResponseMessageLevelV1, ResponseMessageV1, ResponseMetaV1, ResponseV1,
};
use crate::osp_repl::{HistoryConfig, HistoryShellContext, SharedHistory};
use crate::osp_ui::document::Block;
use crate::osp_ui::messages::{MessageBuffer, MessageLevel};
use crate::osp_ui::{RenderSettings, render_output};
...
fn repl_view<'a>(
    runtime: &'a crate::osp_cli::state::AppRuntime,
    session: &'a crate::osp_cli::state::AppSession,
...
fn test_config(entries: &[(&str, &str)]) -> crate::osp_config::ResolvedConfig {
    let mut defaults = ConfigLayer::default();
    defaults.set("profile.default", "default");
    for (key, value) in entries {
        defaults.set(*key, *value);
...
fn synced_render_settings(config: &crate::osp_config::ResolvedConfig) -> RenderSettings {
...
fn render_prompt_snapshot(entries: &[(&str, &str)]) -> String {
    ...
    crate::osp_cli::repl::presentation::build_repl_prompt(repl_view(&state.runtime, &state.session)).left
}

fn render_table_snapshot(entries: &[(&str, &str)]) -> String {
    let config = test_config(entries);
    let mut settings = crate::osp_cli::cli::default_render_settings();
    ...
    let rows = vec![
        crate::osp_cli::row! { "uid" => "alice", "count" => 2 },
        crate::osp_cli::row! { "uid" => "bob", "count" => 15 },
    ];
    render_output(&crate::osp_cli::rows::output::rows_to_output_result(rows), &settings)
}
...
fn sample_catalog() -> Vec<CommandCatalogEntry> {
    vec![CommandCatalogEntry {
        ...
        completion: crate::osp_completion::CommandSpec {
            name: "orch".to_string(),
            tooltip: Some("Provision orchestrator resources".to_string()),
            subcommands: vec![
                crate::osp_completion::CommandSpec::new("provision"),
                crate::osp_completion::CommandSpec::new("status"),
            ],
            ..crate::osp_completion::CommandSpec::default()
        },
    ...
    vec![CommandCatalogEntry {
        ...
        completion: crate::osp_completion::CommandSpec {
            ...
            subcommands: vec![
                crate::osp_completion::CommandSpec::new("provision")
                    .arg(
                        crate::osp_completion::ArgNode::named("guest")
                            .tooltip("Guest name for the provision request"),
                    )
                    .arg(
                        crate::osp_completion::ArgNode::named("image")
                            .tooltip("Base image to provision")
                            .suggestions([
                                crate::osp_completion::SuggestionEntry::from("ubuntu"),
                                crate::osp_completion::SuggestionEntry::from("alma"),
                            ]),
                    )
                    .flag(
                        "--provider",
                        crate::osp_completion::FlagNode::new().suggestions([
                            crate::osp_completion::SuggestionEntry::from("vmware"),
                            crate::osp_completion::SuggestionEntry::from("nrec"),
                        ]),
                    )
                    .flag(
                        "--os",
                        crate::osp_completion::FlagNode {
                            suggestions: vec![
                                crate::osp_completion::SuggestionEntry::from("rhel"),
                                crate::osp_completion::SuggestionEntry::from("alma"),
                            ],
                            suggestions_by_provider: BTreeMap::from([
                                (
                                    "vmware".to_string(),
                                    vec![crate::osp_completion::SuggestionEntry::from("rhel")],
                                ),
                                (
                                    "nrec".to_string(),
                                    vec![crate::osp_completion::SuggestionEntry::from("alma")],
                                ),
                            ]),
                            ..crate::osp_completion::FlagNode::default()
                        },
                    ),
                crate::osp_completion::CommandSpec::new("status"),
            ],
...
fn direct_plugins_command_keeps_clap_action_unit() {
    ...
    assert!(matches!(
        plan.action,
        RunAction::Plugins(crate::osp_cli::cli::PluginsArgs {
...
fn positional_profile_with_config_uses_clap_parser_unit() {
    ...
    match plan.action {
        RunAction::Config(args) => {
            assert!(matches!(
                args.command,
                ConfigCommands::Show(crate::osp_cli::cli::ConfigShowArgs {
                    sources: true,
                    raw: false,
                })
            ));
...
fn repl_dsl_capability_is_declared_per_command_unit() {
    let plugins_list = Commands::Plugins(crate::osp_cli::cli::PluginsArgs {
        command: PluginsCommands::List,
    });
    let plugins_enable = Commands::Plugins(crate::osp_cli::cli::PluginsArgs {
        command: PluginsCommands::Enable(crate::osp_cli::cli::PluginToggleArgs {
            plugin_id: "uio-ldap".to_string(),
        }),
    });
    let theme_show = Commands::Theme(crate::osp_cli::cli::ThemeArgs {
        command: ThemeCommands::Show(crate::osp_cli::cli::ThemeShowArgs { name: None }),
    });
    let theme_use = Commands::Theme(crate::osp_cli::cli::ThemeArgs {
        command: ThemeCommands::Use(crate::osp_cli::cli::ThemeUseArgs {
            name: "nord".to_string(),
        }),
    });
    let config_show = Commands::Config(crate::osp_cli::cli::ConfigArgs {
        command: ConfigCommands::Show(crate::osp_cli::cli::ConfigShowArgs {
            sources: false,
            raw: false,
        }),
    });
    let config_set = Commands::Config(crate::osp_cli::cli::ConfigArgs {
        command: ConfigCommands::Set(crate::osp_cli::cli::ConfigSetArgs {
    ...
    let history_list = Commands::History(crate::osp_cli::cli::HistoryArgs {
        command: crate::osp_cli::cli::HistoryCommands::List,
    });
    let history_prune = Commands::History(crate::osp_cli::cli::HistoryArgs {
        command: crate::osp_cli::cli::HistoryCommands::Prune(crate::osp_cli::cli::HistoryPruneArgs { keep: 5 }),
    });
...
fn external_inline_builtin_reuses_repl_dsl_policy_unit() {
    ...
    let command = Commands::Config(crate::osp_cli::cli::ConfigArgs {
        command: ConfigCommands::Set(crate::osp_cli::cli::ConfigSetArgs {
            key: "ui.mode".to_string(),
            value: "plain".to_string(),
            global: false,
