...
use crate::osp_core::plugin::{DescribeArgV1, DescribeCommandV1, DescribeFlagV1, DescribeSuggestionV1};
...
fn completion_words_collect_flags_and_backbone_commands_unit() {
    let spec = crate::osp_completion::CommandSpec::new("ldap")
        .flag("--json", crate::osp_completion::FlagNode::new())
        .subcommand(
            crate::osp_completion::CommandSpec::new("user")
                .subcommand(crate::osp_completion::CommandSpec::new("show")),
        );
...
fn describe_command_helpers_preserve_nested_completion_metadata_unit() {
    ...
    let command = DescribeCommandV1 {
        ...
        args: vec![DescribeArgV1 {
            ...
            multi: true,
            value_type: Some(crate::osp_core::plugin::DescribeValueTypeV1::Path),
    ...
    assert!(arg.multi);
    assert_eq!(arg.value_type, Some(crate::osp_completion::ValueType::Path));

    let flag = to_flag_node(command.flags.get("--format").expect("flag"));
    assert_eq!(flag.tooltip.as_deref(), Some("output format"));
    assert!(flag.multi);
    assert_eq!(flag.value_type, Some(crate::osp_completion::ValueType::Path));

    let entry = to_suggestion_entry(&suggestion);
    assert_eq!(entry.value, "json");
    assert_eq!(entry.display.as_deref(), Some("JSON"));
    assert_eq!(
        to_value_type(crate::osp_core::plugin::DescribeValueTypeV1::Path),
        Some(crate::osp_completion::ValueType::Path)
    );
}
...
fn search_root_and_checksum_helpers_cover_real_filesystem_paths_unit() {
    ...
    std::fs::write(&exec_path, "#!/usr/bin/env bash\nexit 0\n").expect("exec written");
