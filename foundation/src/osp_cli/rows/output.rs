...
use crate::osp_core::plugin::{ColumnAlignmentV1, ResponseMetaV1};
...
fn response_to_rows(data: serde_json::Value) -> Vec<Row> {
    match data {
        ...
        scalar => vec![crate::osp_cli::row! { "value" => scalar }],
    }
}
...
mod tests {
    ...
    use crate::osp_core::output_model::{ColumnAlignment, Group, OutputItems, OutputMeta, OutputResult};
    use crate::osp_core::plugin::{ColumnAlignmentV1, ResponseMetaV1};
    ...
    fn rows_round_trip_through_output_result_unit() {
        let rows = vec![
            ...
            crate::osp_cli::row! { "uid" => "bob", "count" => 3 },
        ];
    ...
    fn grouped_output_flattens_group_headers_and_rows_unit() {
        let output = OutputResult {
            items: OutputItems::Groups(vec![
                Group {
                    groups: crate::osp_cli::row! { "team" => "ops" },
                    aggregates: crate::osp_cli::row! { "count" => 2 },
                    rows: vec![
                        crate::osp_cli::row! { "user" => "alice" },
                        crate::osp_cli::row! { "user" => "bob" },
                    ],
                },
                Group {
                    groups: crate::osp_cli::row! { "team" => "infra" },
                    aggregates: crate::osp_cli::row! { "count" => 0 },
                    rows: Vec::new(),
                },
            ]),
            meta: OutputMeta {
    ...
    fn plugin_data_scalar_and_object_shapes_are_normalized_unit() {
        ...
        assert_eq!(scalar_rows, vec![crate::osp_cli::row! { "value" => "hello" }]);
        assert_eq!(
            object_rows,
            vec![crate::osp_cli::row! { "uid" => "alice", "count" => 2 }]
        );
    }
}
