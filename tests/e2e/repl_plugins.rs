...
#[cfg(unix)]
fn write_policy_provider_plugin(
    dir: &Path,
    plugin_id: &str,
    command_name: &str,
    visibility: &str,
...
if [ "$1" = "--describe" ]; then
  cat <<'JSON'
{{"protocol_version":1,"plugin_id":"{plugin_id}","plugin_version":"0.1.0","min_osp_version":"0.1.0","commands":[{{"name":"{command_name}","about":"{plugin_id} plugin","auth":{{"visibility":"{visibility}"}},"args":[],"flags":{{}},"subcommands":[]}}]}}
JSON
  exit 0
fi
...
#[cfg(unix)]
#[test]
fn repl_provider_override_keeps_each_selected_plugin_policy_invocation_local() {
    let plugins = make_temp_dir("osp-cli-pty-policy-provider-plugins");
    let _public =
        write_policy_provider_plugin(&plugins, "public-provider", "shared", "public", "public");
    let _guarded = write_policy_provider_plugin(
        &plugins,
        "guarded-provider",
        "shared",
        "authenticated",
    ...
    assert!(
        ...
        "guarded provider should be denied; output:\n{}",
        session.output_snapshot(7000),
    );
    assert!(
        !session
            .output_since(guarded_start)
            .contains("guarded-from-plugin"),
        "guarded provider must not execute; output:\n{}",
        session.output_snapshot(7000),
    );
