...
fn help_like_payload_value_extracts_top_level_usage_array() {
    let output = run_document_pipeline(help_like_guide(), "VALUE usage");

    assert!(GuideView::try_from_output_result(&output).is_none());
    assert!(output.document.is_none());
    let value = result_rows(&output);
    assert_eq!(value, json!([{"value": "osp deploy <COMMAND>"}]));
}
...
fn help_like_payload_value_extracts_nested_entry_field_as_flat_rows() {
    let output = run_document_pipeline(help_like_guide(), "VALUE sections[1].entries[0].name");
...
fn help_like_payload_value_mixed_depth_selectors_keep_stable_order() {
    let output = run_document_pipeline(help_like_guide(), "VALUE usage sections[0].entries[].name");
...
fn help_like_payload_value_extracts_sibling_fields_as_rows() {
    let output = run_document_pipeline(
        help_like_guide(),
